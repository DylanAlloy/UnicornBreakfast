...
Some steps in that .gif in the event that it doesn't load for you:
  ...
  3. Go from "Sources -> Databses" to "Sources -> Tables" and add a table with the name "zika" (MANDATORY and not in the Caravel docs, the table name shouldn't even be a user option!) and select our database's name from the drop down.
  4. Go to your "Tables" page again and click on the edit button in the column and select the tab up top "List Table Column", select "temporal" for the "date_time" column and select everything remaining for the columns left or if you know specifically how you're going to interpret the data beforehand, select only what you need. 
